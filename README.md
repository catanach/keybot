...
- `src/` — the actual code that runs on the Pico (`code.py`), plus a `settings.toml.example` template for your Wi-Fi credentials.
...
3. Run `./deploy.sh` again. This time it copies `code.py`, `settings.toml`, and the two library folders onto the board.
